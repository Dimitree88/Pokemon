// Logica condivisa dell'anteprima carte: indice, sidebar, rendering della pagina.
// Usata da:
//  - scripts/serve.mjs  (dev server: hot-reload, asset serviti dal server, live-reload SSE)
//  - api/index.mjs      (funzione serverless Vercel: asset statici, niente hot-reload)
//
// ROOT = cwd: sia `npm run serve` sia la lambda Vercel girano con cwd = root del
// progetto (dove stanno data/ e assets/). Gli asset grossi (font) NON sono letti qui:
// la pagina li referenzia per URL (serviti staticamente / dal dev server).
...
export const sets = JSON.parse(await readFile(path.join(ROOT, "data", "sets.json"), "utf8"));
...
export const cardIndex = await listCards();
export const firstCardId = cardIndex[0].id;

// Simboli energia + rarità: SVG inline (riletti a ogni render, così rigenerarli con
// `npm run assets` si riflette senza riavviare il dev server).
...
function sidebar(cardId, energy, rarity, setsBase) {
  ...
  for (const g of groups) {
    ...
    const setSym = setSymbols.has(g.setId) ? `<img class="setsym" src="${setsBase}/${g.setId}.png" alt="">` : "";
...
// URL degli asset: nel dev server li serve serve.mjs; su Vercel sono file statici.
const DEFAULT_ASSETS = { cssHref: "/src/card.css", fontsBase: "/assets/fonts", setsBase: "/assets/sets", artBase: "/assets/art" };

// Costruisce la pagina HTML completa (menu + anteprima carta). Ritorna null se la
// carta non esiste. opts: { liveReload, fresh (hot-reload import), assets }.
export async function renderPage(cardId, scale = 1, opts = {}) {
  const { liveReload = false, fresh = false, assets = DEFAULT_ASSETS } = opts;
  ...
  // dev: re-import a caldo (?t=) per riflettere le modifiche a template/font; prod: import statico (bundlato).
  const mod = fresh ? await import(`./card-template.mjs?t=${Date.now()}`) : await import("./card-template.mjs");
  const { fontFaceUrlCss } = fresh ? await import(`./fonts.mjs?t=${Date.now()}`) : await import("./fonts.mjs");
  const { energy, rarity } = await loadSymbols();
  const ctx = {
    energy, rarity,
    artUrl: `${assets.artBase}/${cardId}.png`,
    setSymbolUrl: setSymbols.has(setId) ? `${assets.setsBase}/${setId}.png` : "",
  };
  return mod.buildDocument(card, set, ctx, {
    cssHref: assets.cssHref,
    scale,
    headExtra: `<style>${fontFaceUrlCss(assets.fontsBase)}</style>`,
    bodyPrepend: `<style>${DEV_CSS}</style><div class="dev">${sidebar(cardId, energy, rarity, assets.setsBase)}<main class="viewport">`,
    bodyAppend: `</main></div><script>${clientJS(liveReload)}</script>`,
  });
}
